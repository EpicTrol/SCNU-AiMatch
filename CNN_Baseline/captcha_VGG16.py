...
class CNN2(nn.Module):
    def __init__(self):
        ...
        # self.layer1 = self.make_layers([64, 64, 'M'], 1, droup_out=False)
        self.layer1 = self.make_layers([64, 64, 'M'], 1, droup_out=False)
        # self.layer1 = self.make_layers([32, 32, 'M'], 3, droup_out=False)

        # self.layer2 = self.make_layers([128, 128, 'M'], 64, droup_out=True)
        self.layer2 = self.make_layers([128, 128, 'M'], 64, droup_out=True)
        # self.layer2 = self.make_layers([64, 64, 'M'], 32, droup_out=True)

        self.layer3 = self.make_layers(
            [256, 256, 256, 'M', 512, 512, 512, 'M'], 128, droup_out=False)
        # self.layer3 = self.make_layers(
        #     [128, 128, 128, 'M', 256, 256, 256, 'M'], 64, droup_out=False)
        self.layer4 = self.make_layers(
            [512, 512, 512, 'M'], 512, droup_out=True)
        # self.layer4 = self.make_layers(
        #     [256, 256, 256, 'M'], 256, droup_out=True)
        # self.fc = nn.Sequential(
        #     nn.Linear((IMAGE_WIDTH//32)*(IMAGE_HEIGHT//32)*256, 1024),
